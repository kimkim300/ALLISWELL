...
class MemoApp {
    ...
    bindEvents() {
        ...
        if (addMemoBtn) {
            addMemoBtn.addEventListener('click', () => this.showMemoInputModal());
        }
        
        // 메모 입력 모달 이벤트
        const memoInputModal = document.getElementById('memoInputModal');
        ...
        if (memoInputModal) {
            // 모달 외부 클릭 시 닫기
            memoInputModal.addEventListener('click', (e) => {
                if (e.target.id === 'memoInputModal') {
                    this.hideMemoInputModal();
                }
            });
        }
        
        if (closeMemoModal) {
            closeMemoModal.addEventListener('click', () => this.hideMemoInputModal());
        }
        
        if (saveMemo) {
            saveMemo.addEventListener('click', async () => await this.saveMemoFromModal());
        }
        
        if (cancelMemo) {
            cancelMemo.addEventListener('click', () => this.hideMemoInputModal());
        }
    ...
    async addMemo() {
        // 핸드폰에서는 모달 사용, 데스크톱에서는 기존 방식 사용
        if (window.innerWidth <= 768) {
            this.showMemoInputModal();
            return;
        }
        ...
        if (isNaN(timeValue) || timeValue < 0) {  // 0을 허용하도록 수정
            alert('유효한 소요 시간을 선택해주세요.');
            return;
        }
        ...
        if (this.currentEditId !== null) {
            ...
            const memo = {
                ...
                content: content,
                category: category,
                time: timeValue, // 검증된 timeValue 사용
    ...
    async updateMemo(id, content, category, time) {
        ...
        const timeValue = parseFloat(time);
        if (isNaN(timeValue) || timeValue < 0) {  // 0을 허용하도록 수정
            console.error('유효하지 않은 time 값:', time);
            return;
        }
        ...
        if (memoIndex !== -1) {
            ...
            this.memos[memoIndex].time = timeValue; // 검증된 값 사용
    ...
    // 메모 입력 모달 관련 메서드들
    showMemoInputModal() {
        const modal = document.getElementById('memoInputModal');
        if (modal) {
    ...
    populateModalCategories() {
        ...
        if (modalCategory) {
            modalCategory.innerHTML = '<option value="">카테고리 선택</option>';
    ...
    renderMemos() {
        ...
        // 디버깅: 메모 시간 값과 날짜 값 확인
        displayMemos.forEach(memo => {
            console.log(`메모 렌더링: ${memo.content}`, {
                id: memo.id,
                time: memo.time,
                timeType: typeof memo.time,
                formattedTime: this.formatTimeDisplay(memo.time || 0),
                date: memo.date,
        ...
        memoList.innerHTML = `
            ${displayMemos.map(memo => `
                <div class="memo-item ${memo.important ? 'important' : ''} ${memo.completed ? 'completed' : ''}" data-id="${memo.id}">
                    <div class="memo-tags-container">
                        <div class="memo-category ${memo.category.replace(/\s+/g, '')}" style="${this.getCategoryStyle(memo.category)}">${memo.category}</div>
                        <div class="memo-time">${this.formatTimeDisplay(memo.time || 0)}</div>
                    </div>
                    <div class="memo-header">
                        <div class="memo-date">${memo.date || new Date().toLocaleString('ko-KR')}</div>
            ...
            ${displayRoutines.map(routine => {
                ...
                return `
                    <div class="${routineClass}" data-id="${routine.id}">
                        <div class="routine-tags-container">
    ...
    formatTimeDisplay(timeInHours) {
        ...
        if (timeInHours === undefined || timeInHours === null || isNaN(timeInHours)) {
            console.warn('formatTimeDisplay: 유효하지 않은 시간 값:', timeInHours);
            return '0~30분'; // 기본값 반환
        }
    ...
    showMonthlyStats() {
        ...
        // 통계 계산 및 표시
        console.log(`📊 ${year}년 ${month}월 통계 계산 시작`);
    ...
    calculateMonthlyStats(year, month) {
        ...
        const monthRoutines = this.routines.filter(routine => {
            // 해당 월에 완료된 날짜가 있는 루틴만 포함
            const completedDates = routine.completedDates || [];
            const hasCompletedDatesInMonth = completedDates.some(dateStr => {
                const date = new Date(dateStr);
                return date.getFullYear() === year && date.getMonth() === month - 1;
            });
            
            // 디버깅: 루틴 필터링 정보
            console.log(`루틴 필터링: ${routine.title}`, {
                hasCompletedDatesInMonth: hasCompletedDatesInMonth,
                completedDates: completedDates,
                included: hasCompletedDatesInMonth
            });
            
            return hasCompletedDatesInMonth;
        });
        ...
        const stats = {
            ...
            totalRoutines: 0,
            categories: {},
            monthMemos: monthMemos,  // renderMonthlyStats에서 사용하기 위해 추가
            monthRoutines: monthRoutines  // renderMonthlyStats에서 사용하기 위해 추가
        };
        ...
        monthRoutines.forEach(routine => {
            ...
            const routineTime = this.getUpperBoundTime(routine.time) || 0;
            ...
            // 디버깅: 루틴 정보 출력
            console.log(`루틴 통계 계산: ${routine.title}`, {
                category: routine.category,
                originalTime: routine.time,
                upperBoundTime: routineTime,
                completedDates: completedDates,
            ...
            if (monthCompletedDates.length > 0) {
                ...
                console.log(`✅ 루틴 통계 추가: ${category} - ${totalRoutineTime}시간, ${monthCompletedDates.length}개`);
            } else {
                console.log(`❌ 루틴 통계 제외: ${routine.title} - 완료된 날짜 없음`);
            }
        });
    ...
    renderMonthlyStats(stats) {
        if (stats.totalMemos === 0 && stats.totalRoutines === 0) {
            return `
                <div class="no-stats">
                    <div class="icon">📊</div>
                    <p>이번 달에는 완료된 작업이 없습니다.</p>
                    <p>메모나 루틴을 완료하면 통계에 반영됩니다!</p>
                </div>
            `;
        }
        ...
        const categoryItems = Object.entries(allCategories)
            .map(([category, color]) => {
                ...
                // 메모와 루틴 개수 구분
                const memoCount = stats.monthMemos.filter(memo => memo.category === category).length;
                const routineCount = stats.monthRoutines.filter(routine => routine.category === category).length;
                
                let countText = '';
                if (memoCount > 0 && routineCount > 0) {
                    countText = `${data.count}개 (메모 ${memoCount}개, 루틴 ${routineCount}개)`;
                } else if (memoCount > 0) {
                    countText = `${data.count}개 (메모 ${memoCount}개)`;
                } else if (routineCount > 0) {
                    countText = `${data.count}개 (루틴 ${routineCount}개)`;
                } else {
                    countText = `${data.count}개`;
                }
                ...
                return `
                    <div class="category-stat-item">
                        ...
                        <div class="category-count">${countText} (${percentage}%)</div>
                    </div>
                `;
            }).join('');
        ...
        return `
            <div class="stats-summary">
                ...
                <div class="total-tasks">
                    총 ${stats.totalMemos + stats.totalRoutines}개의 완료된 작업
                    ${stats.totalMemos > 0 ? `(메모 ${stats.totalMemos}개` : ''}
                    ${stats.totalMemos > 0 && stats.totalRoutines > 0 ? ', ' : ''}
                    ${stats.totalRoutines > 0 ? `루틴 ${stats.totalRoutines}개` : ''}
                    ${(stats.totalMemos > 0 || stats.totalRoutines > 0) ? ')' : ''}
                </div>
            </div>
        ...
        try {
            ...
            this.memos.forEach(memo => {
                ...
                groupedMemos[dateStr][category][memo.id] = {
                    ...
                    time: memo.time || 0, // time 필드 추가 및 안전 처리
                    ...
                    Object.keys(groupedMemos).forEach(dateStr => {
                        Object.keys(groupedMemos[dateStr]).forEach(category => {
                            Object.keys(groupedMemos[dateStr][category]).forEach(memoId => {
                                ...
                                // time 필드 안전하게 복원
                                if (memo.time === undefined || memo.time === null || isNaN(memo.time)) {
                                    memo.time = 0; // 기본값 설정
                                }
                                
                                // date 필드 안전하게 복원
                                if (!memo.date || memo.date === 'undefined') {
                                    // dateStr에서 날짜 생성
                                    if (memo.dateStr) {
                                        const memoDate = new Date(memo.dateStr);
                                        memo.date = memoDate.toLocaleString('ko-KR');
                                    } else {
    ...
    async loadDataFromFirebase() {
        ...
        try {
            ...
            // UI 업데이트는 init()에서 처리
            console.log('Firebase 데이터 로드 완료 - UI 렌더링 준비됨');
    ...
    clearUserData() {
        ...
        // 이번달 목표 데이터 제거
        const currentMonth = this.formatDateString(this.currentDate).substring(0, 7);
